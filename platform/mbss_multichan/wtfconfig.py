...
z_ssh.verbosity = 2
ifaces = []
ifaces.append(wtf.node.Iface.create_iface(name="eth1", ops=ops))
ifaces[-1].enable = False
...
#ssh.name = "sniffer"
#ssh.verbosity = 2
#ifaces.append(wtf.node.Iface.create_iface(name="intel", driver="iwl4965", ops=ops))
#ifaces[-1].conf = wtf.node.sniffer.SnifferConf(channel=1, htmode=htmode, iface=ifaces[-1])
#ifaces.append(wtf.node.Iface(name="ath9k", driver="ath9k", ops=ops))
##ifaces[-1].conf = wtf.node.sniffer.SnifferConf(channel=1, htmode=htmode, iface=ifaces[-1])
#sniffer = wtf.node.sniffer.SnifferSTA(ssh, ifaces=ifaces, ops=ops)
